...
export default function SideNav({
    ...
    return (
        ...
        >
            <p className="text-primary text-sm tracking-[0.12em] font-semibold border-b border-border pb-3 md:pb-4">
            ...
            <div className="hidden md:block">
                <h2 className="font-bold mb-2 text-red-500">Topics</h2>
                <FilterSelect
                    value={selectedCategory}
            ...
            <div className="hidden md:block border-t border-border pt-4">
                <h2 className="font-bold mb-2 text-red-500">Universities</h2>
                <FilterSelect
                    value={selectedInstitution}
...
function MobileFilterGroup({ label, value, options, allCount, isOpen, onToggle, onChange, align = 'left' }) {
    ...
    return (
        <div className="relative min-w-0">
            <button
                type="button"
                className="w-full min-h-11 border border-border px-3 py-2 flex items-center justify-between gap-2 text-left font-semibold text-primary bg-white"
            ...
            <ul className="space-y-2 w-full text-primary">
                {visibleOptions.map((o) => (
                    <li key={o.value}>
                        <button
                            type="button"
                            className={`w-full cursor-pointer text-left hover:underline flex items-start justify-between gap-3 ${value === o.value ? 'font-bold underline' : ''}`}
                            aria-pressed={value === o.value}
                            onClick={() => onChange(value === o.value ? 'All' : o.value)}
                        >
                            <span>{o.label}</span>
                            <span className="text-gray-500 tabular-nums">({o.count ?? 0})</span>
                        </button>
                    </li>
                ))}
            </ul>
