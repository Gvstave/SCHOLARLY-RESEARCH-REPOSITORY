...
export default function SubmitterEmailBadge({ email, size = 'md', className = '' }) {
  ...
  if (size === 'sm') {
    return (
      <p className={`text-sm text-gray-500 flex items-center gap-1.5 mt-0.5 ${className}`}>
  ...
  return (
    <p className={`text-sm text-gray-600 flex items-center gap-1.5 mt-2 bg-amber-50/50 border border-amber-100 px-3 py-1.5 w-fit rounded-sm ${className}`}>
