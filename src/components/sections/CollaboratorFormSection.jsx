    ...
    <div className="space-y-6">
      <div className="bg-gray-50 p-4 border border-border">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
          <FormField
            ...
            labelClassName="text-gray-400 tracking-widest text-sm block mb-1"
        ...
        <button
          ...
          className="mt-4 flex items-center gap-1.5 border border-gray-800 px-3.5 py-1.5 text-primary-text text-sm font-semibold hover:bg-gray-800 hover:text-white transition"
        >
      ...
      {collaborators.length > 0 && (
        <div className="space-y-2 border border-border divide-y divide-gray-100">
          <div className="bg-gray-100 px-4 py-2 text-sm text-gray-600 font-bold flex items-center gap-2">
            <Users className="w-3.5 h-3.5 text-gray-500" /> Co-authors on this paper
          </div>
          {collaborators.map((c, idx) => (
            <div key={idx} className="px-4 py-3 flex justify-between items-center text-sm text-primary-text bg-white">
              <div>
                <span className="font-semibold">{c.fullName}</span>
                <span className="text-gray-500 block text-sm mt-0.5">{c.institution} &bull; {c.email}</span>
              </div>
              <button
                type="button"
