...
const CollectionsArchive = () => {
  ...
  const sectionType = location.pathname.includes('/institutions')
    ? 'INSTITUTIONS'
  ...
  const filteredCollections = collections.filter(collection => {
    const matchesSection = collection.sectionType === sectionType;
    ...
    return matchesSection && matchesSearch && matchesInstitution;
  });

  const sectionTitle = sectionType === 'INSTITUTIONS'
    ? 'Institutional Repositories'
    : sectionType === 'CURATIONS'
    ? 'Curated Programs'
    : 'Thematic Archives';

  const sectionDescription = sectionType === 'INSTITUTIONS'
    ? 'Browse major institutional collections from partner repositories and research libraries.'
    : sectionType === 'CURATIONS'
    ? 'Explore editorially curated research programs and thematic resource bundles.'
    : 'Explore broad thematic archives organized by field, era, and scholarly focus.';
    ...
    <div className="min-h-screen bg-background text-on-surface">
      ...
      <main className="pt-32 pb-20 px-8 max-w-screen-xl mx-auto">
        ...
        <header className="mb-16 max-w-3xl">
          <h1 className="font-headline text-5xl font-extrabold tracking-tighter text-primary mb-6">{sectionTitle}</h1>
        ...
        <section className="mb-12 flex flex-col md:flex-row gap-6 items-end border-b border-outline-variant/15 pb-8">
          ...
          <button
            className="bg-slate-900 text-white px-8 py-3 uppercase tracking-[0.3em] text-xs font-bold hover:bg-slate-700 transition-colors"
        ...
        <div className="mt-24 flex items-center justify-between border-t border-outline-variant/30 pt-12">
          <span className="font-label text-[10px] uppercase tracking-[0.3em] text-outline">Showing {filteredCollections.length.toString().padStart(2, '0')} of {totalCollections.toString().padStart(3, '0')} Collections</span>
          <div className="flex gap-12">
            <button className="font-headline font-bold text-sm tracking-widest uppercase text-slate-500 hover:text-slate-900 transition-colors">Previous</button>
            <button className="font-headline font-bold text-sm tracking-widest uppercase text-slate-900 border-b-2 border-slate-900 pb-1">Next Page</button>
          </div>
        </div>
      </main>
